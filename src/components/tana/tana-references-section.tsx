...
export function TanaReferencesSection({
  ...
  return (
    <section aria-label="引用此节点" className="mt-8 border-t pt-5">
      <h2 className="mb-2 flex items-center gap-2 font-medium text-sm text-[#435148]">
        <Link2Icon aria-hidden="true" className="size-4 text-[#6f8879]" />
      ...
      <div className="space-y-0.5">
        {references.map((reference, position) => {
          ...
          return (
            ...
            >
              <span aria-hidden="true" className="size-1.5 shrink-0 rounded-full bg-[#83a894]" />
              <span className="min-w-0 flex-1 truncate">{title || '未命名节点'}</span>
              <span className="shrink-0 text-[#929a95] text-[10px]">
                {reference.kind === 'inline' ? '行内' : '节点'}
              </span>
              <ArrowUpRightIcon
                aria-hidden="true"
                className="size-3 shrink-0 text-[#9aa19d] opacity-0 transition-opacity group-hover:opacity-100"
              />
            </button>
          );
        })}
