  ...
  switch (expression.type) {
    ...
    case "or":
      return expression.children.every((child) =>
        isTanaQueryExpressionValid(index, child),
      );
  }
}
  ...
  switch (predicate.kind) {
    ...
    case "referenced-by":
      return (
        index.backlinks
          .get(node.id)
          ?.some((reference) => reference.sourceNodeId === predicate.nodeId) ??
        false
      );
  }
}
...
export function matchesTanaQueryExpression(
  ...
  switch (expression.type) {
    ...
    case "and":
      return expression.children.every((child) =>
        matchesTanaQueryExpression(node, index, child),
      );
    case "or":
